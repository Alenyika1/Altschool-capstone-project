...
resource "azurerm_kubernetes_cluster" "eks" {
  ...
  identity {
    type         = "UserAssigned"
    identity_ids = [azurerm_user_assigned_identity.capstone.id]
  }
  ...
  depends_on = [
    azurerm_role_assignment.capstone
  ]
}
